<div class="container-fluid section-document p-5">
    <div class="d-flex align-content-center justify-content-center img-course montserrat-bold h1"
         style="font-size: 51px; color: #207143">
        Tài liệu
    </div>
    <div class="main-content pb-5 d-none d-md-block">
        <div class="row pt-5 gy-3">
            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a target="_blank" href="{{asset('pdf/i7smart0711.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/simoni7.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%">
                        <div class=" montserrat-bold  pb-2 pt-4"
                            style="font-size: 22px; color: #207143;">
                            <div class="text-center" style=" border-bottom: 4px solid #207143; padding-bottom: 1px;">Cartalouge i7 Smart</div>
                        </div>
                    </a>
                </div>
            </div>

            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a target="_blank" href="{{asset('pdf/CatalogueSimonProject2024.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/switch.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 20px; color: #207143;">
                            <div class="text-center" style=" border-bottom: 4px solid #207143; padding-bottom: 1px;">SWITCHES & SOCKETS</div>
                        </div>
                    </a>
                </div>
            </div>

            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a target="_blank" href="{{asset('pdf/Catalogue-Lumi.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/lumi.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%;">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 22px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">Nhà thông minh Lumi</div>
                        </div>
                    </a>
                </div>
            </div>


            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a target="_blank" href="{{asset('pdf/i7smart0711.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/simoni7.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 22px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">Cartalouge i7 Smart</div>
                        </div>
                    </a>
                </div>
            </div>

            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a target="_blank" href="{{asset('pdf/CatalogueSimonProject2024.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/switch.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 20px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">SWITCHES & SOCKETS</div>
                        </div>
                    </a>
                </div>
            </div>

            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a target="_blank"  href="{{asset('pdf/Catalogue-Lumi.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/lumi.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%;">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 22px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">Nhà thông minh Lumi</div>
                        </div>
                    </a>
                </div>
            </div>



            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a target="_blank" href="{{asset('pdf/i7smart0711.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/simoni7.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 22px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">Cartalouge i7 Smart</div>
                        </div>
                    </a>
                </div>
            </div>

            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a  target="_blank"  href="{{asset('pdf/CatalogueSimonProject2024.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/switch.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 20px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">SWITCHES & SOCKETS</div>
                        </div>
                    </a>
                </div>
            </div>

            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a  target="_blank"  href="{{asset('pdf/Catalogue-Lumi.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/lumi.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%;">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 22px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">Nhà thông minh Lumi</div>
                        </div>
                    </a>
                </div>
            </div>
        </div>
    </div>


    <div class=" pb-5 d-block d-md-none">
        <div class="row pt-5 gy-3">
            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a href="{{asset('pdf/i7smart0711.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/simoni7.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 22px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">Cartalouge i7 Smart</div>
                        </div>
                    </a>
                </div>
            </div>

            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a href="{{asset('pdf/CatalogueSimonProject2024.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/switch.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 20px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">SWITCHES & SOCKETS</div>
                        </div>
                    </a>
                </div>
            </div>

            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a href="{{asset('pdf/Catalogue-Lumi.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/lumi.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%;">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 22px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">Nhà thông minh Lumi</div>
                        </div>
                    </a>
                </div>
            </div>


            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a href="{{asset('pdf/i7smart0711.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/simoni7.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 22px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">Cartalouge i7 Smart</div>
                        </div>
                    </a>
                </div>
            </div>

            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a href="{{asset('pdf/CatalogueSimonProject2024.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/switch.png')}}" alt="hoa-tien-image" loading="lazy"  class="img-fluid"
                             style="object-fit: cover; height: 80%">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 20px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">SWITCHES & SOCKETS</div>
                        </div>
                    </a>
                </div>
            </div>

            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a href="{{asset('pdf/Catalogue-Lumi.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/lumi.png')}}" alt="" class="img-fluid"
                             style="object-fit: cover; height: 80%;">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 22px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">Nhà thông minh Lumi</div>
                        </div>
                    </a>
                </div>
            </div>



            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a href="{{asset('pdf/i7smart0711.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/simoni7.png')}}" alt="" class="img-fluid"
                             style="object-fit: cover; height: 80%">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 22px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">Cartalouge i7 Smart</div>
                        </div>
                    </a>
                </div>
            </div>

            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a href="{{asset('pdf/CatalogueSimonProject2024.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/switch.png')}}" alt="" class="img-fluid"
                             style="object-fit: cover; height: 80%">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 20px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">SWITCHES & SOCKETS</div>
                        </div>
                    </a>
                </div>
            </div>

            <div class="col-lg-4 col-xs-12 d-flex justify-content-center" style="height: 45vh; ">
                <div class="w-100 d-flex justify-content-center pt-3" style="background-color: #F5F5F5">
                    <a href="{{asset('pdf/Catalogue-Lumi.pdf')}}" style="text-decoration: unset">
                        <img src="{{asset('pdf/lumi.png')}}" alt="" class="img-fluid"
                             style="object-fit: cover; height: 80%;">
                        <div class=" montserrat-bold  pb-2 pt-4"
                             style="font-size: 22px; color: #25AAE2;">
                            <div class="text-center" style=" border-bottom: 4px solid #25AAE2; padding-bottom: 1px;">Nhà thông minh Lumi</div>
                        </div>
                    </a>
                </div>
            </div>
        </div>
    </div>
</div>
<style>
    .main-content {
        padding: 0 25vh;
    }
</style>
